...
class ENL_Mailer
{
    ...
    /** Envia para todos ou um destinatário. Chama $on_log($payload) ao final. */
    public function send_to_all_subscribers(int $post_id, ?string $only_email, callable $on_log): bool
    {
        ...
        foreach ($recipients as $email) {
            ...
            $sent ? $ok++ : ($fail++ && $fail_list[$email] = 'wp_mail retornou false');
        }
    ...
    /** Envia o último post (para uso no cadastro) */
    ...
    /** Prepara URL/CID da destacada e armazena path para AddEmbeddedImage */
    ...
    /** SMTP + debug + embed CID + transient para tela “Verificar Envio” */
    private function smtp_send(string $email, string $subject, string $body): bool
    {
        ...
        // monta resumo para a aba Verificar Envio
        $mask = function ($s) {
            if (!$s) return '';
            $l = strlen($s);
